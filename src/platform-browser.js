(function () {
  ...
  const learningTracks = [
    ...
    },
    {
      ...
      items: [
        { type: 'workflow', title: 'GitHub 倉庫使用與成果整理', description: '1.0 先用手動方式提交學習影片截圖、組裝照片、測試結果與心得。', url: 'https://github.com/' }
      ]
    }
  ];
  ...
  window.FdePlatform = {
    ...
    simulateBuildAssistant(question = '') {
      ...
      return {
        ...
        topic,
        answer: 'RAG 知識庫尚未接入。1.0 示範回答：請先確認 Pixhawk 箭頭朝向機頭，再檢查 GPS 羅盤、電源模組、接收機接線、馬達旋轉方向與 CW / CCW 槳葉是否和組裝規則一致。',
    ...
    simulateHoverAssessment(fileName = 'hover.mp4') {
      return {
        type: 'hover-video',
        engine: 'YOLOv8 hover scoring placeholder',
        fileName,
