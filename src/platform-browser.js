window.FdePlatform = {
  hero: {
    ...
    summary: '以 AI 輔助教學為核心，串接學習影片、模擬訓練、實物組裝、YOLOv8 檢測與 GitHub 成果存證。',
    status: 'v1.1 frontend prototype'
  },
  moduleSections: [
    { key: 'home', label: '首頁', title: '平台總覽', summary: '透明藍色無人機啟動畫面與整體平台介紹。' },
    { key: 'learn', label: '學', title: '影片學習與資料下載', summary: '集中放置 F450 組裝、Mission Planner 地面站與飛控基礎資源。' },
    { key: 'practice', label: '練', title: '模擬器安裝與飛行練習', summary: '引導使用者下載模擬器並完成起飛、懸停、轉向與降落練習。' },
    { key: 'build', label: '做', title: 'F450 實物組裝與 AI 問答', summary: '用 SOP 完成實物組裝，並預留 RAG 知識庫與 GPT 語音排錯入口。' },
    { key: 'assess', label: '測', title: '槳葉正反面 YOLOv8 檢測', summary: '上傳照片後模擬檢測 CW / CCW 槳葉方向，後續接入真實模型。' },
    { key: 'certify', label: '證', title: 'GitHub 成果存證', summary: '整理學習紀錄、組裝證據、檢測結果與專案報告，上傳至 GitHub。' }
  ],
  learningResources: [
    {
      type: 'youtube',
  ...
  simulateBuildAssistant(question = '') {
    const topic = question.trim() || '組裝問題';
    return {
      mode: 'rag-gpt-voice-placeholder',
      topic,
      answer: 'RAG 知識庫尚未接入。示範回答：請先確認機頭方向，再檢查飛控主板、GPS 羅盤與槳葉 CW / CCW 是否和組裝圖一致。',
      voiceStatus: 'GPT 語音回答介面已預留，尚未接入真實語音服務。'
    };
  },
  simulatePropellerAssessment(fileName = 'propeller-check.jpg') {
    return {
